    ...
    <v-container fluid style="padding:8px">
        ...
        <v-list two-line>
            ...
            <v-list-tile>
              ...
              <v-chip color="primary" text-color="white" style="width:100px;" @click="addNewIdea">
                <span class="pointer">Ajouter</span>
              <v-icon right class="pointer" style="margin-left:10px">add</v-icon>
            </v-chip>
            ...
            <v-list-tile v-for="(idea, index) in ideas" :key="index" @click="" v-if="!hasBeenReportedByUser(index)">
              ...
              <v-list-tile-content style="max-width:155px;position:absolute; right:5px">
                <v-tooltip top v-if="idea.developed">
                  <v-chip slot="activator" color="success" text-color="white" style="width:100px;font-size:10px;padding-left:7px" class="pointer">
                  développée
                </v-chip>
                  <span>Cette fonctionnalité est maintenant disponible sur  <span class="primary--text">nouvomo</span></span>
                </v-tooltip>
                <v-tooltip top v-else-if="idea.inDevelopment">
                  <v-chip slot="activator" color="accent" text-color="white" style="width:100px;font-size:8px" class="pointer">
                  en développement
                </v-chip>
                  <span>Cette fonctionnalité est en cours de développement.</span>
                </v-tooltip>
                <span v-else @click.prevent="report(index, idea)" style="text-decoration:underline;font-size:13px;opacity:0.6;text-align:right;padding-right:10px;width:100%" class="secondary--text">signaler</span>
              </v-list-tile-content>
            </v-list-tile>
  ...
  <v-dialog v-model="modalReported" max-width="290">
    <v-card class="card-modal">
      <v-card-title class="headline secondary--text">
        Vous venez de signaler cette idée
      </v-card-title>
      <v-card-text class="secondary--text" style="margin-top:-15px">
        Merci d'avoir contribué à améliorer la qualité du contenu de <span class="primary--text">nouvomo</span>.
      </v-card-text>
      </v-card>
    </v-dialog>
